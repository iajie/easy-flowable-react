...
import { Button, Modal, Space, Popover, Dropdown, MenuProps, Upload, Typography } from "antd";
import { DownloadOutlined, EyeOutlined, ImportOutlined, KeyOutlined, RedoOutlined, ReloadOutlined, SaveOutlined, ZoomInOutlined, ZoomOutOutlined } from "@ant-design/icons";
...
const Toolbar: React.FC<ToolbarProps> = ({ modeler, save, style = toolbarDefaultStyle, ...props }) => {
	...
	const xmlSave = async () => {
		if (save) {
			...
			// 更新流程信息
			xmlResult.xml && await save(xmlResult.xml);
		}
	}

	const viewXml = async () => {
		const xmlResult = await modeler.saveXML({ format: true });
		xmlResult.xml && Modal.info({
	...
	return <div style={style}>
		<Modal title="快捷键" open={shortcutKeysOpen} width={"25%"} centered
		...
		<Space>
			...
			<Space.Compact block>
				<Popover content="导入xml文件">
					<Upload accept=".xml" showUploadList={false} onChange={({ file }) => {
						if (file.status === 'done') {
				...
				<Dropdown menu={{ items }}><Button icon={<DownloadOutlined />}></Button></Dropdown>
			</Space.Compact>
			{save && <Popover content="保存更新">
				<Button icon={<SaveOutlined />} onClick={() => xmlSave()} />
			</Popover>}
			{props.title ? props.title : <Button type="link" style={{ color: 'GrayText' }} size="large">Easy-Flowable流程设计器</Button>}
		</Space>
	</div>
}
